...
class Citizen extends FlxSprite {
	...
	public function new(X:Float = 0, Y:Float = 0, State:PlayState = null) {
		...
		taskTimer.start(random.float(WAIT_TIME_MIN, WAIT_TIME_MAX), getNewTask, 1);
		waiting = true;
	...
	public function addHair():Void {
		/*
			...
			hair.animation.play(Std.string(random.intRanged(0, 5)));
	...
	private function createAnimations():Void {
		...
		// this.scale.y = random.floatRanged(0.9, 1.1);
	...
	private function updateStats():Void {
		if (random.sign(statDecreaseChance) > 0) {
			var decrease = random.int(0, 3);
			// FlxG.log.add(name + " stat decrease");
			switch (decrease) {
				case 0:
					angerStat += random.float(0.5, 3);
				case 1:
					danceStat -= random.float(0.5, 3);
				case 2:
					sleepStat -= random.float(0.5, 3);
				case 3:
					exersiceStat -= random.float(0.5, 3);
			}
		}
		...
		if (badStats >= 3 && random.sign(statDecreaseChance) > 0) {
			health -= random.float(0.05, 0.25);
	...
	private function getNewTask(t:FlxTimer):Void {
		if (waiting && !dying) {
			waiting = false;
			if (random.sign(walkChance) > 0) {
				walking = true;
				walkTargetX = random.float(50, FlxG.width - 50);
				// FlxG.log.add(name + " Starting walk " + walkTargetX);
			} else if (random.sign(fightChance) > 0 && state.citizens.countLiving() >= 2) {
			...
			} else if (random.sign(danceChance) > 0) {
				danceTime = 0;
				dancing = true;
			} else if (random.sign(workoutChance) > 0) {
				workoutTime = 0;
				workingout = true;
			} else if (random.sign(sleepChance) > 0) {
				sleepTime = 0;
				sleeping = true;
			} else if (random.sign(raveChance) > 0) {
				raving = true;
				danceTime = 0;
			} else if (random.sign(slamChance) > 0) {
				slamming = true;
				danceTime = 0;
			} else if (random.sign(zombieChance) > 0) {
				zombieing = true;
				zombieTime = 0;
			}
		}
	...
	private function suicide():Bool {
		...
		if (random.sign(suicideChance) > 0) {
			health = 0;
	...
	private function fight():Bool {
		...
		if (random.sign(punchChance) > 0) {
			fightTarget.health -= random.float();
	...
	private function dance():Bool {
		...
		if (danceTime > maxDanceTime / 4 * 3 && random.sign(danceDeathChance) > 0) {
			this.health = 0;
			dancing = false;
	...
	private function workout():Bool {
		...
		if (random.sign(statDecreaseChance) > 0) {
			exersiceStat += random.float(3, 8);
			if (exersiceStat > 100) {
				exersiceStat = 100;
			}
		}

		if (workoutTime > maxWorkoutTime * .85 && random.sign(workoutDeathChance) > 0) {
			this.health = 0;
			workingout = false;
		...
		if (workoutTime > maxWorkoutTime || random.sign(workoutQuitChance) > 0) {
			workingout = false;
			workoutTime = 0;
			return false;
		}
	...
	private function sleep():Bool {
		...
		if (random.sign(statDecreaseChance) > 0) {
			health += random.float(0.05, 0.5);
			if (health > 1) {
				health = 1;
			}
			sleepStat += random.float(3, 8);
			if (sleepStat > 100) {
				sleepStat = 100;
			}
		}

		if (sleepTime > maxSleepTime || random.sign(wakeUpChance) > 0) {
			sleeping = false;
			sleepTime = 0;
			return false;
		}
	...
	private function slam():Bool {
		...
		if (random.sign(punchChance) > 0) {
			...
			if (tries < 5) {
				target.health -= random.float(0.05, .25);
	...
	private function zombie():Bool {
		...
		if (random.sign(punchChance) > 0) {
			...
			if (tries < 5 && Math.abs(FlxMath.distanceBetween(this, zombieTarget)) < 100) {
				zombieTarget.health -= random.float(0.05, .25);
