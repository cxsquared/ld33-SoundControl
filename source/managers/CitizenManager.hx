...
class CitizenManager extends FlxTypedGroup<Citizen> {
	...
	public function addCitizens(ammount:Int = 1):Void {
		for (i in 0...ammount) {
			var citizen = new Citizen(random.float(50, FlxG.width - 50), 200, state);
			add(citizen);
		}
		scored = false;
		loops = 0;
	...
	private function updateDance(t:FlxBasic):Void {
		...
		if (SoundManager.soundLevels.Chill > .55) {
			citizen.danceChance = Math.max(baseDanceChance * Math.pow(SoundManager.soundLevels.Dance * 2 - SoundManager.soundLevels.Chill / 1.5, 2),
				baseChance);
		} else {
	...
	private function updateWorkout(t:FlxBasic):Void {
		var citizen = cast(t, Citizen);
		citizen.workoutChance = Math.max(baseWorkoutChane * Math.pow(((SoundManager.soundLevels.Dance * 2) + (SoundManager.soundLevels.Chill * 2)) / 2, 2),
			baseChance);
